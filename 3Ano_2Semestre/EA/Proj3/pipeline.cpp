...
bool compareIdSmaller(Operation* op1, Operation* op2) {
    if (op1->id > op2->id)
        return false;
    else
        return true;
...
bool compareIdBigger(Operation* op1, Operation* op2) {
    if (op1->id < op2->id)
        return false;
    else
        return true;
...
// vectorOption -> 0 for nodesVisited; 1 for nodesChecked
bool checkDependencies(Operation* operation, int vectorOption) {
    if (vectorOption == 0) {
...
vector<int> findBottlenecks(int id, int vecId, vector<int> res) {
    ...
    vector<Operation*> childVec = vector<Operation*>();

    for (Operation* op : operations[id]->childrens) {
        if (nodesChecked[op->id] == false && checkDependencies(op, 1))
            childVec.push_back(op);
    }

    // sort
    sort(childVec.begin(), childVec.end(), &compareIdBigger);

    for (Operation* op : childVec) {
        if (nodesChecked[op->id] == false)
            res = findBottlenecks(op->id, vecId, res);
    }
    return res;
}
...
void runPipelineSingleThreaded() {
    ...
    while (!queue.empty()) {
        ...
        sort(queue.begin(), queue.end(), &compareIdSmaller);
        ...
        for (Operation* op : queue[0]->childrens) {
            if (nodesVisited[op->id] == false && checkDependencies(op, 0))
                queue.push_back(op);
        }
...
int main() {
    ...
    switch (option) {
        ...
        case 3:
            ...
            res = findBottlenecks(idInitOp, 0, res);
